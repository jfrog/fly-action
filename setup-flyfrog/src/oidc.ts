...
export function extractUserFromToken(token: string): string | undefined {
    try {
        ...
        if (parts.length !== 3) throw new Error('Unable to extract user from OIDC token');
 ...
 */
export async function authenticateOidc(url: string): Promise<{ user: string; accessToken: string }> {
    ...
    if (!credentials.accessToken) throw new Error('Token exchange did not return an access token');
