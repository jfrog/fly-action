...
/**
 * Integration tests for the fly-darwin-arm64 binary.
 * These tests verify that the binary is functional and accepts expected arguments.
 *
 * Note: These tests run against the actual binary and require:
 * - macOS ARM64 platform (or the appropriate binary for your platform)
...
describe("Fly Binary Integration Tests", () => {
  const binPath = getBinaryPath();

  describe("Binary existence and permissions", () => {
    it("should have the binary file present", () => {
...
describe("Binary cross-platform check", () => {
  it("should have all platform binaries present", () => {
  ...
  it("should have consistent version across all binaries", () => {
    const binDir = path.resolve(__dirname, "..", "bin");
    ...
    // We can only test the current platform's binary
